...
export default async (req,res) => {
    ...
    do {
        ...
        unique = await User.countDocuments({username:username}).catch((err) => {
            ...
            .json(errorJson(err.message, "An internal server error while counting username"))
        });
    }while(unique);
    ...
    user = await user.save().catch((err) => {
        return res
            .status(500).json(errorJson(err.message,"An internal server error while registering you."))
    });
